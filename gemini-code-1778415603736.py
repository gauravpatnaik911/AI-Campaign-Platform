...
def inject_studio_aesthetic():
    ...
    <style>
        ...
        [data-testid="stVerticalBlockBorderWrapper"]:hover { box-shadow: 0 8px 30px rgba(0,0,0,0.06) !important; }
        div[data-testid="stVerticalBlock"] div[style*="border"] { padding: 24px !important; border: none !important; box-shadow: none !important; }

        /* Primary Buttons */
        button[kind="primary"] {
            background-color: var(--primary-orange) !important; color: #FFFFFF !important; border: none !important; border-radius: 8px !important;
            font-weight: 600 !important; padding: 0.6rem 1.2rem !important; transition: all 0.2s ease;
        }
        
        /* Secondary Buttons (Quick Starts) */
        button[kind="secondary"] {
            text-align: left !important; justify-content: flex-start !important;
            background-color: var(--main-bg) !important; color: var(--text-muted) !important;
            border: 1px solid var(--border-color) !important; border-radius: 8px !important;
            font-weight: 500 !important; font-size: 13px !important; transition: all 0.2s ease; padding: 0.8rem 1rem !important;
        }
        button[kind="secondary"]:hover { background-color: #fff !important; color: var(--primary-orange) !important; border-color: var(--primary-orange) !important; }
        
        /* Sleek Popover Button (Artifacts) */
        button[data-testid="stPopoverButton"] {
            border: 1px dashed #c1c5c8 !important; background-color: transparent !important; color: var(--text-muted) !important;
            font-weight: 600 !important; justify-content: center !important; border-radius: 8px !important; padding: 0.5rem !important; transition: all 0.2s ease;
        }
        button[data-testid="stPopoverButton"]:hover { border-color: var(--primary-orange) !important; color: var(--primary-orange) !important; background-color: #fff !important; }
...
PERSONAS = [
    "Creative Designer (Ops)", "Marketing Professional (Ops)", "Merchandiser (Ops)", 
    "Supply Chain Manager (Ops)", "Analyst / Data Scientist (Analytical)", "Brand Strategy Leader (Strategy)"
]
...
def execute_backend_script(script_name: str, args: list):
    try:
        if not os.path.exists(script_name): return True
...
if not st.session_state.auto_intelligence_generated:
    ...
    col_dash, col_chat = st.columns([3, 1.3], gap="large")
            ...
            with st.container(border=True):
                if "Designer" in sel_per and any(f.name.endswith(('png', 'jpg')) for f in files):
                    ...
                    with c1:
                        ...
                        st.json(ctx_json)
                        if st.button("Generate Imagen 3", type="primary", use_container_width=True):
    ...
    # ==================== RIGHT PANE: ENCAPSULATED CHAT AI ====================
    with col_chat:
        ...
        with st.container(border=True):
            ...
            with chat_container:
                ...
                if len(st.session_state.chat_history) <= 1:
                    ...
                    for i, qs in enumerate(doc.get("chatQuickStart", [])):
                        if st.button(f"{qs}", key=f"qs_{i}", type="secondary", use_container_width=True):
            ...
            with st.popover("➕ Attach Artifacts", use_container_width=True):
                ...
                if uploaded_files:
                    if st.button("Process Assets", type="primary", use_container_width=True):
            ...
            if prompt or st.session_state.pending_prompt:
                ...
                if prompt:
                    st.session_state.chat_history.append({"role": "user", "content": actual_prompt})
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("""
        <div style='text-align: left; color: var(--text-muted); font-size: 0.8rem; font-weight: 600; border-top: 1px solid var(--border-color); padding-top: 1.5rem; text-transform: uppercase; letter-spacing: 0.05em;'>
